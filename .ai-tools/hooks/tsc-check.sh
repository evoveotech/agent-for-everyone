...
# Uses stderr for visibility in AI assistant Code main interface

AI_PROJECT_DIR="${AI_PROJECT_DIR:-$HOME/project}"
HOOK_INPUT=$(cat)
SESSION_ID="${session_id:-default}"
CACHE_DIR="$HOME/.ai-tools/tsc-cache/$SESSION_ID"
...
get_repo_for_file() {
    local file_path="$1"
    local relative_path="${file_path#$AI_PROJECT_DIR/}"
...
run_tsc_check() {
    local repo="$1"
    local repo_path="$AI_PROJECT_DIR/$repo"
...
find "$HOME/.ai-tools/tsc-cache" -maxdepth 1 -type d -mtime +7 -exec rm -rf {} \; 2>/dev/null || true
