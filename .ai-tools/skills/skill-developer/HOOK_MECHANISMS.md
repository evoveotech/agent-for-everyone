...
User submits prompt
    ↓
.ai-tools/settings.json registers hook
    ↓
...
Outputs formatted message to stdout
    ↓
stdout becomes context for AI assistant (injected before prompt)
    ↓
AI assistant sees: [skill suggestion] + user's prompt
```

### Key Points

- **Exit code**: Always 0 (allow)
- **stdout**: → AI assistant's context (injected as system message)
- **Timing**: Runs BEFORE AI assistant processes prompt
- **Behavior**: Non-blocking, advisory only
- **Purpose**: Make AI assistant aware of relevant skills
...
AI assistant sees this output as additional context before processing the user's prompt.
...
```
AI assistant calls Edit/Write tool
    ↓
.ai-tools/settings.json registers hook (matcher: Edit|Write)
    ↓
...
ELSE:
  Exit with code 0 (ALLOW)
    ↓
IF BLOCKED:
  stderr → AI assistant sees message
  Edit/Write tool does NOT execute
  AI assistant must use skill and retry
IF ALLOWED:
...
- **Exit code 2**: BLOCK (stderr → AI assistant)
- **Exit code 0**: ALLOW
...
AI assistant receives this message and understands it needs to use the skill before retrying the edit.
...
| Exit Code | stdout | stderr | Tool Execution | AI assistant Sees |
|-----------|--------|--------|----------------|-------------|
...
1. **Only way** to send message to AI assistant from PreToolUse
2. stderr content is "fed back to AI assistant automatically"
3. AI assistant sees the block message and understands what to do
...
Prevent repeated nagging in the same session - once AI assistant uses a skill, don't block again.

### State File Location

`.ai-tools/hooks/state/skills-used-{session_id}.json`
...
- If AI assistant doesn't use the skill but makes a different edit, it won't block again
- Trust that AI assistant follows the instruction
