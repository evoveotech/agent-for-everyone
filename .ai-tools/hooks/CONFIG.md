...
### 1. Register Hooks in .ai-tools/settings.json

Create or update `.ai-tools/settings.json` in your project root:
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "$AI_PROJECT_DIR/.ai-tools/hooks/stop-prettier-formatter.sh"
          },
          {
            "type": "command",
            "command": "$AI_PROJECT_DIR/.ai-tools/hooks/stop-build-check-enhanced.sh"
          },
          {
            "type": "command",
            "command": "$AI_PROJECT_DIR/.ai-tools/hooks/error-handling-reminder.sh"
          }
        ]
      }
    ]
...
Edit `.ai-tools/hooks/post-tool-use-tracker.sh`, function `detect_repo()`:
...
Edit `.ai-tools/hooks/post-tool-use-tracker.sh`, function `get_build_command()`:
...
Edit `.ai-tools/hooks/post-tool-use-tracker.sh`, function `get_tsc_command()`:
...
Edit `.ai-tools/hooks/stop-prettier-formatter.sh`, function `get_prettier_config()`:
...
Configure file category detection in `.ai-tools/hooks/error-handling-reminder.ts`:
...
Edit `.ai-tools/hooks/stop-build-check-enhanced.sh`:
...
Set before starting AI assistant Code:
...
```
$AI_PROJECT_DIR/.ai-tools/tsc-cache/[session_id]/
```
...
rm -rf $AI_PROJECT_DIR/.ai-tools/tsc-cache/*

# Remove specific session
rm -rf $AI_PROJECT_DIR/.ai-tools/tsc-cache/[session-id]
```
...
1. **Check registration:** Verify hook is in `.ai-tools/settings.json`
2. **Check permissions:** Run `chmod +x .ai-tools/hooks/*.sh`
3. **Check path:** Ensure `$AI_PROJECT_DIR` is set correctly
4. **Check TypeScript:** Run `cd .ai-tools/hooks && npx tsc` to check for errors
...
View hook execution in AI assistant Code's logs.
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "$AI_PROJECT_DIR/.ai-tools/hooks/my-custom-bash-guard.sh"
          }
        ]
      }
    ]
...
4. **Version control** - Commit `.ai-tools/` directory to git
