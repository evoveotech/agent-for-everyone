...
**Check:**
```bash
cat .ai-tools/skills/skill-rules.json | jq .
```
...
```
- Editing: `frontend/src/components/Dashboard.tsx` → 
- Editing: `frontend/tests/Dashboard.test.tsx` →  (add exclusion!)
- Editing: `backend/src/app.ts` → 
...
```
- Editing: `services/user.test.ts` → 
- Editing: `services/user.ts` → 
...
```
- File has: `import { PrismaService } from './prisma'` → 
- File has: `import { Database } from './db'` → 
...
**Check session state:**
```bash
ls .ai-tools/hooks/state/
cat .ai-tools/hooks/state/skills-used-{session-id}.json
```
...
```bash
rm .ai-tools/hooks/state/skills-used-{session-id}.json
```
...
```bash
cat <<'EOF' | npx tsx .ai-tools/hooks/skill-verification-guard.ts 2>&1
{
  "session_id": "debug",
  "tool_name": "Edit",
...
**Check `.ai-tools/settings.json`:**
```bash
cat .ai-tools/settings.json | jq '.hooks.UserPromptSubmit'
cat .ai-tools/settings.json | jq '.hooks.PreToolUse'
```
    ...
    "UserPromptSubmit": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "$CLAUDE_PROJECT_DIR/.ai-tools/hooks/skill-activation-prompt.sh"
          }
        ]
      }
    ]
...
**Check:**
```bash
head -1 .ai-tools/hooks/skill-activation-prompt.sh
```
...
```bash
# UserPromptSubmit
time echo '{"prompt":"test"}' | npx tsx .ai-tools/hooks/skill-activation-prompt.ts

# PreToolUse
time cat <<'EOF' | npx tsx .ai-tools/hooks/skill-verification-guard.ts
