...
Specialized sub-agents for complex, multi-step engineering tasks.

---

## What Are Agents?

Agents are autonomous AI assistant instances that handle specific complex tasks. Unlike capabilities (which provide inline guidance), agents:
...
cp showcase/.ai-tools/agents/agent-name.md \\
   your-project/.ai-tools/agents/
```

**Step 2: Verify (optional)**
```bash
# Check for hardcoded paths
grep -n "~/git/\|/root/git/\|/Users/" your-project/.ai-tools/agents/agent-name.md
```

**Step 3: Use it**
Ask your AI assistant: "Use the [agent-name] agent to [task]"
...
- Update to use `$AI_PROJECT_DIR` or relative paths
...
3. **Check for hardcoded paths:**
   ...
   ```
4. **Update paths if found** to `$AI_PROJECT_DIR` or `.`
...
ls -la .ai-tools/agents/[agent-name].md
```
...
**Check for hardcoded paths:**
```bash
grep "~/\|/root/\|/Users/" .ai-tools/agents/[agent-name].md
```

**Fix:**
```bash
sed -i 's|~/git/.*project|$AI_PROJECT_DIR|g' .ai-tools/agents/[agent-name].md
```
...
3. **Ask your AI assistant to use them** - "Use [agent] to [task]"
