# EvoVeo AI Integration Guide

**FOR AI ASSISTANTS:** When a user asks you to integrate components from the EvoVeo AI Engineering Infrastructure Reference Framework into their project, follow these instructions carefully.

---

## Overview

This framework is a **reference collection** of AI-assisted engineering infrastructure components. Users will ask you to help integrate specific pieces into their projects. Your role is to:
...
## Enterprise Tech Stack Compatibility Check
...
```bash
cp -r /path/to/showcase/.ai-tools/skills/[skill-name] \\
      $AI_PROJECT_DIR/.ai-tools/skills/
```
...
**Check if it exists:**
```bash
ls $AI_PROJECT_DIR/.ai-tools/skills/skill-rules.json
```
...
ls -la $AI_PROJECT_DIR/.ai-tools/skills/[skill-name]

# Validate skill-rules.json syntax
cat $AI_PROJECT_DIR/.ai-tools/skills/skill-rules.json | jq .
```
...
1. **Copy the skill as a starting point:**
   ```bash
   cp -r showcase/.ai-tools/skills/frontend-dev-guidelines \\
         $AI_PROJECT_DIR/.ai-tools/skills/vue-dev-guidelines
   ```
...
```bash
# Copy both files
cp showcase/.ai-tools/hooks/skill-activation-prompt.sh \\
   $AI_PROJECT_DIR/.ai-tools/hooks/
cp showcase/.ai-tools/hooks/skill-activation-prompt.ts \\
   $AI_PROJECT_DIR/.ai-tools/hooks/

# Make executable
chmod +x $AI_PROJECT_DIR/.ai-tools/hooks/skill-activation-prompt.sh

# Install dependencies if needed
if [ -f "showcase/.ai-tools/hooks/package.json" ]; then
  cp showcase/.ai-tools/hooks/package.json \\
     $AI_PROJECT_DIR/.ai-tools/hooks/
  cd $AI_PROJECT_DIR/.ai-tools/hooks && npm install
fi
```
...
```bash
# Copy file
cp showcase/.ai-tools/hooks/post-tool-use-tracker.sh \\
   $AI_PROJECT_DIR/.ai-tools/hooks/

# Make executable
chmod +x $AI_PROJECT_DIR/.ai-tools/hooks/post-tool-use-tracker.sh
```
...
```bash
# Copy the agent file
cp showcase/.ai-tools/agents/[agent-name].md \\
   $AI_PROJECT_DIR/.ai-tools/agents/
```
...
- `~/git/old-project/` → Should be `$AI_PROJECT_DIR` or `.`
- `/root/git/project/` → Should be `$AI_PROJECT_DIR` or `.`
- Hardcoded screenshot paths → Ask user where they want screenshots

**If found, update them:**
```bash
sed -i 's|~/git/old-project/|.|g' $AI_PROJECT_DIR/.ai-tools/agents/[agent].md
sed -i 's|/root/git/.*PROJECT.*DIR|$AI_PROJECT_DIR|g' \\
    $AI_PROJECT_DIR/.ai-tools/agents/[agent].md
```
...
```bash
# Copy command file
cp showcase/.ai-tools/commands/[command].md \\
   $AI_PROJECT_DIR/.ai-tools/commands/
```
...
ls -la $AI_PROJECT_DIR/.ai-tools/hooks/*.sh
...
ls $AI_PROJECT_DIR/.ai-tools/hooks/node_modules/
# Should show packages if package.json exists

# 4. Settings.json is valid JSON
cat $AI_PROJECT_DIR/.ai-tools/settings.json | jq .
# Should parse without errors
```
...
Do you have a .ai-tools/settings.json file already, or should I create one?
```
...
I'll copy it to your .ai-tools/agents/ directory now.
```
...
## EvoVeo Engineering Standards for AI Assistants
